...
def createModel(Input, labels):
    # create model
    model = Sequential()
    model.add(Dense(10, input_dim=10, init='normal', activation='linear'))# normal for guassian distribution
    ...
    history = LossHistory()
    model.fit(Input, labels, nb_epoch=3400, batch_size=100, verbose=2 ,callbacks=[history])
    losslist=[]
...
for row in index:
    ...
    for da in range(0,6):
        ...
        data[da].append(val)
        data[da+6].append(val*val)
        i += 1

Input = data[0:5]+data[6:11]
for i in range(0, 10):
    Input[i] = preprocessing.scale(Input[i], with_mean=True, with_std=True)
    #poly[i] = preprocessing.scale(poly[i], with_mean=True, with_std=True)
...
testData = [list(), list(), list(), list(), list(), list(), list(), list(), list(), list()]
...
for row in range(2, nRows):
    if index.__contains__(row) == False:
        ...
        for d in range(0,5):
            val = sheetName.cell_value(row, i)
            testData[d].append(val)
            testData[d+5].append(val*val)
            i += 1
# print testData
for i in range(0, 10):
